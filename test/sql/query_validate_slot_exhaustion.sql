...
\! for i in $(seq 1 40); do psql -d contrib_regression -c "SELECT pg_sleep(3), COUNT(*) FROM orders LIMIT 1;" > /dev/null 2>&1 & done
...
WHERE backend_pid <> pg_backend_pid() AND query_text LIKE '%pg_sleep(3)%';
