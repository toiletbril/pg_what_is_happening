...
\! curl -s http://localhost:9187/metrics > /tmp/pwh_endpoint_metrics.txt

\! grep "^pg_what_is_happening_active_query_node_time_percent{" /tmp/pwh_endpoint_metrics.txt | head -1 | sed 's/.*query_id="\([^"]*\)".*/\1/' > /tmp/pwh_query_id.txt
\! test -s /tmp/pwh_query_id.txt && grep "query_id=\"$(cat /tmp/pwh_query_id.txt)\"" /tmp/pwh_endpoint_metrics.txt | grep "time_percent{" | awk '{print $NF}' | awk '{s+=$1} END {print s}' > /tmp/pwh_time_sum.txt || echo "0" > /tmp/pwh_time_sum.txt
SELECT
  (SELECT pg_read_file('/tmp/pwh_time_sum.txt')::text::float8) BETWEEN 95 AND 105 AS time_percent_sums_to_100_per_query;

\! grep "^pg_what_is_happening_active_query_node" /tmp/pwh_endpoint_metrics.txt | grep "node_tag=\"Hash" | wc -l > /tmp/pwh_hash_count.txt
SELECT (SELECT pg_read_file('/tmp/pwh_hash_count.txt')::text::int) > 0 AS has_hash_node_metrics;

\! grep "^pg_what_is_happening_active_query_node" /tmp/pwh_endpoint_metrics.txt | grep "node_tag=\"Sort" | wc -l > /tmp/pwh_sort_count.txt
