...
\set time_sum `tr -d '[:space:]' < /tmp/pwh_time_sum.txt`
SELECT
  :'time_sum'::float8 BETWEEN 95 AND 105 AS time_percent_sums_to_100_per_query;
...
\set hash_count `tr -d '[:space:]' < /tmp/pwh_hash_count.txt`
SELECT :'hash_count'::int > 0 AS has_hash_node_metrics;
...
\set sort_count `tr -d '[:space:]' < /tmp/pwh_sort_count.txt`
SELECT :'sort_count'::int > 0 AS has_sort_node_metrics;
