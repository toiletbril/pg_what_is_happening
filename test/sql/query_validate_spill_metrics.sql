\! psql -d contrib_regression -c "SET work_mem = '64kB'; SELECT pg_sleep(0.01), o.order_id, SUM(p.amount) FROM orders o JOIN payments p ON o.order_id = p.order_id GROUP BY o.order_id ORDER BY SUM(p.amount) DESC;" > /dev/null 2>&1 &
