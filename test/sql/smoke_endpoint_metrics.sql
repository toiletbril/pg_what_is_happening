...
\! PGAPPNAME=pwh_endpoint_smoke psql -X -v ON_ERROR_STOP=1 -d contrib_regression -c "SELECT pg_advisory_lock(12347), COUNT(*) FROM orders o JOIN users u ON o.user_id = u.user_id JOIN products p ON o.user_id = p.product_id JOIN reviews r ON p.product_id = r.product_id; SELECT pg_advisory_unlock(12347);" > /dev/null 2>&1 &

SELECT pg_sleep(0.2);

-- Query the /metrics endpoint while the query is running.
-- We should see OpenMetrics output with node-level metrics.
\! curl --fail --silent --show-error --max-time 5 http://localhost:9187/metrics > /tmp/pwh_metrics_output.txt
...
\set metric_count `pid=$(psql -X -At -d contrib_regression -c "SELECT pid FROM pg_stat_activity WHERE application_name = 'pwh_endpoint_smoke'"); grep "pid=\"$pid\"" /tmp/pwh_metrics_output.txt | grep -c "^pg_what_is_happening_active_query_node_.*{query_id="`
SELECT
  :metric_count > 96 AS has_sufficient_metrics;

-- Check HTTP status code is 200.
\set http_code `curl --silent --max-time 5 -o /dev/null -w "%{http_code}" http://localhost:9187/metrics`
SELECT
