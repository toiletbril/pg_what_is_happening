...
\! curl -fsS http://localhost:9187/metrics | grep -q 'query_text=""' && echo true > /tmp/pwh_default_redaction.txt
...
\! curl -fsS http://localhost:9187/metrics | grep -q 'query_text="SELECT pg_sleep(2);"' && echo true > /tmp/pwh_explicit_exposure.txt
